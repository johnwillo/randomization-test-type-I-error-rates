...
Create a data frame with combinations of sample sizes and standard deviations for the two samples, n1 and n2. Standard deviations for each sample are specified in columns s1 and s2. Add column p.randomization and fill with NA. The mean type I error values for these columns will be filled in by the for loop below.
...
The combination of 1000 simulations and 1000 permutations per test in the for loop below takes about 13.5 minutes to run.
...
type.1.error = mean(replicate(nreps, 
          twoSamplePermutationTestLocation(rsnorm(combos2$n1[i], mean = 0, sd = combos2$s1[i], xi = 3), rsnorm(combos2$n2[i], mean = 0, sd = combos2$s2[i], xi = 3), n.permutations = 1000)$p.value) < 0.05)


combos2[i, 5] = type.1.error
